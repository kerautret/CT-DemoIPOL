...
void SegmentationBasedOnComponentTree::DoSegmentationWithPreviousComponentTree (double alpha, QImage &segmentationResult, Timer timers[]){
    DoSegmentationWithPreviousComponentTree(alpha, timers);
...
void SegmentationBasedOnComponentTree::DoSegmentationWithPreviousComponentTree (double alpha, QImage &segmentationResult){
    DoSegmentationWithPreviousComponentTree(alpha);

    convertLibTIMToQImage (resultImage, segmentationResult, transparency);
}


void SegmentationBasedOnComponentTree::DoSegmentationWithPreviousComponentTree (double alpha, Timer timers[]){
    timers[3].StartCycle ();
...
void SegmentationBasedOnComponentTree::DoSegmentationWithPreviousComponentTree (double alpha){
    tree -> m_root -> calpha = computeCAlpha (tree -> m_root, alpha);
    computeResult (alpha);
}
...
void SegmentationBasedOnComponentTree::Compute (double alpha, QImage &input, QImage &marker, QImage &segmentationResult){
    convertQImageToLibTIM (input, inputImage);
    convertQImageToLibTIM (marker, markerImage);
    if (tree)
        delete tree;
    tree = new ComponentTree<U8> (inputImage, markerImage);
    DoSegmentationWithPreviousComponentTree (alpha, segmentationResult);
}
...
void SegmentationBasedOnComponentTree::Compute (double alpha, QImage &input, QImage &marker, QImage &segmentationResult, QImage &contour){
    Compute(alpha, input, marker, segmentationResult);
...
void SegmentationBasedOnComponentTree::ComputeComponentTreeOnly (QImage &input, QImage &marker){
    convertQImageToLibTIM (input, inputImage);
    convertQImageToLibTIM (marker, markerImage);
    if (tree)
        delete tree;
...
unsigned short SegmentationBasedOnComponentTree::GetInputWidth (){
    return width;
}

unsigned short SegmentationBasedOnComponentTree::GetInputHeight (){
    return height;
}
